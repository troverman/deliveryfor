# -*- coding: utf-8 -*-
"""
    modified version gaema.auth to simplify and interact with web2py
    ~~~~~~~~~~
...
class RequestAdapter(object):
    ...
    def __init__(self, request):
        ...
        for k in request.vars:
            self.arguments.setdefault(k, []).append(request.vars[k])
        self.host = request.env.http_host
        self.path = request.env.request_uri
        self.web2py_request = request

    def full_url(self):
        return 'http://'+self.host+self.web2py_request.url
...
class BaseAuth(object):
    ...
    def redirect(self, url):
        ...
        :return:
            `None`.
        """
        self.settings['globals']['redirect'](url)
    ...
    def get_argument(self, name, default=_ARG_DEFAULT, strip=True):
        ...
        """
        value = self._request.vars.setdefault(name, default)
    ...
    def get_cookie(self, name, default=None):
        ...
        :return:
            The cookie value.
        """
        if self._request.cookies.has_key(name):
            return self._request.cookies[name].value
        else:
            return default
    ...
    def set_cookie(self, name, value, domain=None, expires=None, path='/',
        ...
        :return:
            `None`.
        """
        self._response.cookies[name] = value
        if expires:
            self._response.cookies[name]['expires'] = expires
        elif expires_days:
            #convert days to seconds
            self._response.cookies[name]['expires'] = expires_days * 24 * 3600
        if path:
            self._response.cookies[name]['path'] = path
        if domain:
            self._response.cookies[name]['domain'] = domain
...
class TwitterMixin(OAuthMixin):
    ...
    def _on_twitter_request(self, callback, response):
        if response.error:
            logging.warning("Error response %s fetching twitter request",
                            response.error)
            callback(None)
            return
...
class FacebookMixin(object):
    ...
    def get_authenticated_user(self, callback):
        ...
        """
        self.require_setting("facebook_api_key", "Facebook Connect")
        session = json_decode(self.get_argument("session"))
